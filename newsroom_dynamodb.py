...
def insert_article(metadata: Dict[str, Any]) -> bool:
    ...
    try:
        ...
        try:
            existing = table.get_item(Key={'PK': item['PK'], 'SK': item['SK']})
            if 'Item' in existing:
...
def fetch_articles_by_date_range(
    ...
    try:
        ...
        response = table.query(
            ...
            KeyConditionExpression=Key('date_key').between(
                f"DATE#{date_from}",
                f"DATE#{date_to}"
            ),
        ...
        for item in response.get('Items', []):
            articles.append(_dynamodb_item_to_dict(item))
...
def fetch_recent_articles(days_back: int = 7, limit: int = 500) -> List[Dict[str, Any]]:
    """
...
def generate_facets(days_back: int = 90) -> Dict[str, Any]:
    ...
    try:
        ...
        response = table.scan(
            ...
            ProjectionExpression='core_topics, source, pub_date',
            Limit=10000
        )
